...
class BlogController(private val blogService: BlogService) {
    ...
    @GetMapping(path = ["/{id}"])
    fun readById(@PathVariable id: Long): Blog {
        return blogService.readById(id)
    }

    @PreAuthorize(value = "@blogServiceImpl.isOwner(authentication, #id)")
    @PutMapping(path = ["/{id}"])
    fun update(@PathVariable id: Long, @RequestBody blogRequest: BlogRequest) {
        blogService.update(id, blogRequest)
    }
}
